package Net::GereTPs::V1::Documents;
...
sub new{
  ...
  if(defined $params->{delivery}){
    $self->{delivery} = $params->{delivery};

    #FIX ME
      $self->{service}{api}{documents}{url} = "http://localhost:3000/api/deliveries/$self->{delivery}/documents";
  }else{
    #FIX ME
      $self->{service}{api}{documents}{url} = "http://localhost:3000/api/documents";
  }
...
sub get{
  ...
  return decode_json _curl("GET", "$self->{service}{api}{documents}{url}/$id", $data);
}
...
sub get_xml{
  ...
  return _curl("GET", "$self->{service}{api}{documents}{url}/$id.xml", $data);
}
...
sub download{
  ...
  return _curl("GET", "$self->{service}{api}{documents}{url}/$id/download", $data);
}
...
sub all{
  ...
  return decode_json _curl("GET", $self->{service}{api}{documents}{url}, $data);
}
...
Net::GereTPs::V1::Documents - Perl extension for GereTPs Documents APIv1
